import React from 'react';
import Fab from '@material-ui/core/Fab';
import AddIcon from '@material-ui/icons/Add';

import MoviesTable from '../MoviesTable/MoviesTable';
import MoviesForm from '../MoviesForm/MoviesForm';

import withHocs from './MoviesHoc';

class Movies extends React.Component {
  state = {
    open: false,
    name: '',
    genre: '',
    watched: false,
    rate: 0,
    directorId: '',
    disabled: true
  }

  handleClickOpen = (data = {}) => {
    this.setState({
      open: true,
      ...data,
      directorId: data.director ? data.director.id : '',
    }, this.handleValidation);

  };

  handleClose = () => {
    this.setState({
      name: '',
      genre: '',
      watched: false,
      rate: 0,
      directorId: '',
      open: false,
      disabled: true
    });
  };

  handleSelectChange = ({ target }) => { this.setState({ [target.name]: target.value }, this.handleValidation ) };

  handleChange = name => ({ target: {value, checked} }) => {
    let newValue = name !== 'watched' ? value : checked

    this.setState({ [name]: newValue }, this.handleValidation);
  };

  handleValidation = () => {
    const {name, genre, rate, directorId} = this.state
    let arr = [name, genre, String(rate), directorId]

    this.setState({disabled: arr.every(item => item.length > 0) ? false : true})

  }


  render() {

    const { id, name, genre, watched, rate, directorId, open, disabled } = this.state;
    const { classes } = this.props;

    return (
      <>
        <MoviesForm
            handleChange={this.handleChange}
            handleSelectChange={this.handleSelectChange}
            selectedValue={{ ...this.state }}
            open={open}
            onClose={this.handleClose}
            disabled = {disabled}
        />
        <div className={classes.wrapper}>
          <MoviesTable onOpen={this.handleClickOpen} onClose={this.handleClose} />
          <Fab onClick={() => this.handleClickOpen()} color="primary" aria-label="Add" className={classes.fab}>
            <AddIcon />
          </Fab>
        </div>
      </>
    );
  }
};

export default withHocs(Movies)
